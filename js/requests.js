...
fetch('http://127.0.0.1:1234/similarity_scores?reference=' + encodeURIComponent(document.getElementById("referenceComp").value) + '&synd_comp=' + document.getElementById("syndromeComp").value + '&facial_subregion=' + network.getSelectedNodes()[0])
...
uploadMesh = () => {
  ...
  fetch('http://127.0.0.1:1234/uploadMesh', {
    method: "POST",
    ...
    body: formData})
        ...
        .then(data => {
          ...
          fetch('http://127.0.0.1:1234/getNormals')
          .then(function (body) {
              return body.json();
          })
          .then(data =>{
            normals = data
            console.log(data)
            submissionScene.meshes[0].setVerticesData(BABYLON.VertexBuffer.NormalKind, normals)
          })
          //get classifier bar plot
          //a note for future me. This approach doesn't scale to multiple users because I assume the registered mesh hasn't been overwritten in the backend. If we expect simultaneous registrations, we would need to stream the mesh back to the server for the classification/personal morphospace request.
          fetch('http://127.0.0.1:1234/classifyMesh?selected.sex=' + document.getElementById('submissionSex').value + '&selected.age=' + document.getElementById('ageInput').value)
          .then(function (body) {
          ...
          colors = [];
          normals = []
          for(var p = 0; p < submissionScene.meshes[0].getVerticesData(BABYLON.VertexBuffer.PositionKind).length; p++) {
              colors.push(211/255, 211/255, 211/255, 1);
              // normals.push(submissionScene.meshes[0].getVerticesData("normal")[p+1])
          }

          submissionScene.meshes[0].setVerticesData(BABYLON.VertexBuffer.ColorKind, colors)
          // submissionScene.meshes[0].setVerticesData(BABYLON.VertexBuffer.NormalKind, normals)
